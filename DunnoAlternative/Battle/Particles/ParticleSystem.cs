...
namespace DunnoAlternative.Battle.Particles
{
    ...
    internal class ParticleSystem
    {
        ...
        public void Draw(RenderWindow window)
        {
            foreach (var p in ParticlePool)
            {
                if (p.RemainingLife > 0)
                {
                    ...
                    ParticleShape.Rotation = p.Rotation * 180 / MathF.PI;
        ...
        public void Emit(ParticleProp prop)
        {
            ...
            p.Velocity = new Vector2f(
                    prop.Velocity.X + prop.VelocityVariation.X * (Global.random.NextSingle() - 0.5f),
                    prop.Velocity.Y + prop.VelocityVariation.Y * (Global.random.NextSingle() - 0.5f)
                );

            p.SizeStart = prop.SizeStart + prop.SizeVariation * (Global.random.NextSingle() - 0.5f);
            p.SizeEnd = prop.SizeEnd + prop.SizeVariation * (Global.random.NextSingle() - 0.5f);

            p.Life = prop.Life + prop.LifeVariation * (Global.random.NextSingle() - 0.5f);
            p.RemainingLife = p.Life;
            p.RotationSpeed = prop.RotationSpeed + prop.RotationSpeedVariation * (Global.random.NextSingle() - 0.5f);
